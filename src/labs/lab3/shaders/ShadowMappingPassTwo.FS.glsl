...
vec3 SpotLight()
{
    ...
    if (spot_light > cos(cut_off)) {
        ...
        // PhongLight - diffuse + specular light
        vec3 phong_lighting = PhongLight();
        float shadow = ShadowFactor();

        vec3 diffuse_and_specular = shadow * phong_lighting;
        return KA + light_att_factor * light_intensity * diffuse_and_specular;
    }
