...
Lab2::Lab2()
{
    no_of_generated_points = no_of_instances = 0;
    max_translate = max_rotate = 0.f;
    control_p0 = control_p1 = control_p2 = control_p3 = glm::vec3(0);
    operation_surface = curve_type = 0;
}
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    GLint control_p0_loc = glGetUniformLocation(shader->program, "control_p0");
    glUniform3f(control_p0_loc, control_p0.x, control_p0.y, control_p0.z);

    GLint control_p1_loc = glGetUniformLocation(shader->program, "control_p1");
    glUniform3f(control_p1_loc, control_p1.x, control_p1.y, control_p1.z);

    GLint control_p2_loc = glGetUniformLocation(shader->program, "control_p2");
    glUniform3f(control_p2_loc, control_p2.x, control_p2.y, control_p2.z);

    GLint control_p3_loc = glGetUniformLocation(shader->program, "control_p3");
    glUniform3f(control_p3_loc, control_p3.x, control_p3.y, control_p3.z);

    GLint no_of_instances_loc = glGetUniformLocation(shader->program, "no_of_instances");
    glUniform1i(no_of_instances_loc, no_of_instances);
    ...
    GLint no_of_generated_points_loc = glGetUniformLocation(shader->program, "no_of_generated_points");
    glUniform1i(no_of_generated_points_loc, no_of_generated_points);

    GLint max_translate_loc = glGetUniformLocation(shader->program, "max_translate");
    glUniform1f(max_translate_loc, max_translate);

    GLint max_rotate_loc = glGetUniformLocation(shader->program, "max_rotate");
    glUniform1f(max_rotate_loc, max_rotate);

    GLint operation_surface_loc = glGetUniformLocation(shader->program, "translate_surface");
    glUniform1i(operation_surface_loc, operation_surface);

    GLint curve_type_loc = glGetUniformLocation(shader->program, "curve_type");
    glUniform1i(curve_type_loc, curve_type);
...
void Lab2::OnKeyPress(int key, int mods)
{
    ...
    {
        no_of_generated_points--;
        if (no_of_generated_points < 2)
        {
            no_of_generated_points = 2;
        }
    }
    // Translation
    if (key == GLFW_KEY_B)
